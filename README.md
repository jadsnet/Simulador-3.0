...
<p align="center">
  <img alt="Versão" src="https://img.shields.io/badge/versão-V7.10.10-E75493">
...
- [x] Pré-validação do Storage evita dezenas de downloads 400 para arquivos órfãos
- [x] Sincronização baixa somente imagens realmente existentes no bucket
- [x] Botão de sincronização usa ícone vetorial de nuvem com fundo transparente
...
- Proteção contra questão inexistente durante atualizações assíncronas.
- Uploads simultâneos do mesmo banco deduplicados e lote interrompido no primeiro erro do Storage.
...
- Barra de rolagem do índice redesenhada nos temas normal e Hello Kitty.
...
- Borda da foto em Perfil e metas removida e botão superior refinado.

- Foto da tela de perfil redesenhada com proporção menor e acabamento circular.
- Fundo azul residual removido quando existe uma foto cadastrada.

- Textos formatados renderizados corretamente no gerenciamento de bancos.
- Tags internas removidas dos resumos, buscas, flashcards e anotações.
- Marcações antigas de tamanho de fonte são limpas automaticamente.

- Correção da aplicação de negrito e itálico no editor visual.
- Seleção preservada depois da escolha da cor e barra reposicionada.
- Controles de aumento e redução de fonte removidos.
...
Cada usuário autenticado agora possui um IndexedDB separado pelo seu UUID do Supabase. Ao trocar de conta, o contexto anterior é descartado e o aplicativo força a restauração dos bancos, histórico, progresso, nível, XP e metas pertencentes exclusivamente à nova conta.
